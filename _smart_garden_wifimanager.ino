...
IrrigationMode decideMode() {
  ...
  if (isDry && isHot)  { g_lastReason = "Kho + nong - phun suong"; return MODE_MIST; }
  if (isDry && !isHot) { g_lastReason = "Kho + binh thuong - tuoi goc"; return MODE_WATER; }
  g_lastReason = "Dat du am - nghi";
  return MODE_IDLE;
}
    ...
    if (client.available()) {
      ...
      if (c == '\n') {
        if (currentLine.length() == 0) {
          ...
          if (header.indexOf("GET /mode/auto")   >= 0) manualMode = false;
          if (header.indexOf("GET /mode/manual") >= 0) manualMode = true;
...
void printSerialJSON() {
  ...
  Serial.printf(
    "{\"temp\":%.1f,\"humi\":%.0f,\"soil\":%d,\"light\":%d,\"water\":%s,\"mist\":%s,\"mode\":\"%s\",\"reason\":\"%s\"}\n",
...
void readSerialCommand() {
  ...
  cmd.trim();
  if (cmd == "WATER_ON")   { manualMode = true; manualWater = true; }
  if (cmd == "WATER_OFF")  { manualMode = true; manualWater = false; }
  if (cmd == "MIST_ON")    { manualMode = true; manualMist = MIST_ON; }
  if (cmd == "MIST_OFF")   { manualMode = true; manualMist = MIST_OFF; }
